...
if ($id > 0) {
    ...
    $stmt = mysqli_prepare($conn, "UPDATE requests SET status = 'Rejected', remarks = 'Rejected by barangay staff. Please contact the barangay office for clarification.' WHERE id = ?");
    mysqli_stmt_bind_param($stmt, 'i', $id);
    mysqli_stmt_execute($stmt);

    if ($request) {
        notify_user($conn, (int)$request['user_id'], 'Your ' . $request['request_type'] . ' request (' . $request['reference_number'] . ') was rejected. Please contact the barangay office.');
    }
}
